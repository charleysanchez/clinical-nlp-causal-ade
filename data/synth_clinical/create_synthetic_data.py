...
def main(
    ...
    for i, r in base_df.iterrows():
        ...
        if r["target"] == 1:
            ...
            if (not is_train and random.random() < 0.7) or (
                is_train and random.random() < 0.5
            ):
                # implicit
    ...
    df[["doc_id", "hadm_id", "subject_id", "split", "label", "T", "AKI"]].to_csv(
        data_dir / "doc_labels.csv", index=False
    )

    version_fn = "data/synth_clinical/VERSION"
